    ...
    <div className="card bg-gray-800 shadow-2xl mt-20 mx-4 md:mx-auto md:max-w-4xl">
      <figure>
        <img
          src={image.url}
          alt={image.title}
          className="pt-10 w-11/12 h-full object-cover"
        />
      </figure>
      <div className="card-body">
        <div className="flex items-center mb-4">
          <img
            src={`http://127.0.0.1:8000/photos_profile/${image.user.pictures}`}
            alt={image.user.id}
            className="w-10 h-10 rounded-full mr-4"
          />
          <div>
            <h3 className="text-lg font-bold mb-1">{image.user.name}</h3>
          </div>
        </div>
        <div className="card-actions justify-center space-x-4">
          ...
          >
            <FaHeart
              className={`text-lg ${liked ? "text-red-500" : "text-white"}`}
            />
